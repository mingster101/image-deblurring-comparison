...
existing = set(os.path.splitext(f)[0] for f in os.listdir(OUTPUT) if f.endswith('.png'))
files = [f for f in files if os.path.splitext(os.path.basename(f))[0] not in existing]
print(f'Missing: {len(files)} gambar yang akan diproses')
